...
*.VERSION @(#) $Id:$
*.HISTORY
*     Sun Feb  9 03:40:12 CLST 2003 upgraded for new LOSSAM
...
c      read(50,*)                ! skip comment line
c      read(50,9999) fileSatel    ! file containing the limit info etc
...
c----------------------------------------------------------------------
     ...
     $     '/lasilla/dimm/meteo.html'
      ...
      maxUtDiff = .1            ! max difference between UT from meteo
...
C--   Transparency
      if (debug) write(6,*)"DBG: start transp"
      ...
      if (y_max.gt.abs1) then
         y(1) = abs1            ! abs warning line
         y(2) = abs1
         ...
         call sm_expand(expand_small)
         call sm_putlabel(9,' \t Clouds?')
         call sm_ltype(0)      ! solid
      endif


      if (y_max.gt.abs2) then
         x(1) = hour_left       ! abs warning line
         x(2) = hour_now
         y(1) = abs2
         y(2) = abs2
      ...
      call sm_ctype(default_col(idev)) 
      call sm_ptype(dot,npt)   ! plot the curve
      call sm_expand(2.)
      call sm_points(hour,xabs,npt)


      if (xabs(1) .lt. abs1 ) then ! chose the color of the big dot
         call sm_ctype('green')
         abscol = green
      else if (xabs(1).lt. abs2)  then
         call sm_ctype('yellow')
         abscol = yellow
      else
         call sm_ctype('red')
      ...
      call sm_points(hour(1),xabs(1),1)
      call sm_ctype(default_col(idev))
      call sm_expand(1.001)


      label20 = 'Extinction (mg/airm)'
      call labeline(iut,hour_now,hour_left,y_min,y_max,label20)
      call sm_expand(expand_def)    

      if (xabs(1).le.0.) then
         call sm_ctype('yellow')
         call sm_relocate(hour_now,0.03)
         call sm_expand(expand_small)    
         call sm_putlabel(7,'\t Currently not available ')
         call sm_ctype(default_col(idev))
         call sm_expand(expand_def)    
...
C--   Transparency RMS
      if (debug) write(6,*)"DBG:   start rms"
      ...
      if (y_max.gt.abs_rms1) then
         y(1) = abs_rms1            !  warning line
         y(2) = abs_rms1
         call sm_ctype(linecol) 
         call sm_conn(x,y,2)
         call sm_relocate(x(1),y(1)+(y_max-y_min)*label_off)
      ...
      if (y_max.gt.abs_rms2) then
         x(1) = hour_left       ! abs warning line
         x(2) = hour_now
         y(1) = abs_rms2
         y(2) = abs_rms2

c         call sm_ltype(1)      ! dots
         call sm_ctype(linecol) 
         call sm_conn(x,y,2)
         call sm_relocate(x(1),y(1)+(y_max-y_min)*label_off)
         call sm_expand(expand_small)
      ...
      call sm_expand(2.)
      call sm_ctype(default_col(idev))
      call sm_points(xabs_ut,xabs_rms,nabs)

      if (xabs_rms(nabs) .lt. abs_rms1 ) then ! chose the color of the big dot
         call sm_ctype('green')
         abs_rmscol = green
      else if (xabs_rms(nabs).lt. abs_rms2)  then
         call sm_ctype('yellow')
         abs_rmscol = yellow
      else
         call sm_ctype('red')
      ...
      call sm_points(xabs_ut(nabs),xabs_rms(nabs),1)

      call sm_ctype(default_col(idev))
      call sm_expand(1.001)

      label20 = 'Extinction RMS'
      call labeline(iut,hour_now,hour_left,y_min,y_max,label20)
      call sm_expand(expand_def)    

      if (xabs(1).le.0..or.
     |    xabs_rms(1).le.0.) then
         call sm_ctype('yellow')
         call sm_relocate(hour_now,0.003)
         call sm_expand(expand_small)    
         call sm_putlabel(7,'\t Currently not available ')
         call sm_ctype(default_col(idev))
         call sm_expand(expand_def)    
...
!-- bottom labels 
      ...
      if (idev .eq. 2) then
         ...
         call sm_expand(expand_small)
         call sm_putlabel
     |        (1,'\t Meteomonitor 2.5; comments: ohainaut@eso.org')
...
c--   fill the table
 ...
 5070 format(
     ...
     | '     <td>        </td>'/
     | '     <td ALIGN=left COLSPAN=2>arcsec</td>'/
     | ' </tr>  '/
     |)
...
c     | '<hr>'/
     | '    ---    <font color="red"><b>Links: </b></font> ['/
     | '<a href="/lasilla/Telescopes/',
     | '2p2T/EnvMon/1p54/">',
     | ' Danish 1.54m Tel. Environment Monitor </a>|'/
     | '<a href="/lasilla/Telescopes/',
     | '2p2T/EnvMon/2p2/">',
     | 'ESO/MPI 2.2m Tel. Environment Monitor</a>|',
     | '<a href="/">ESO/La Silla</a>|'/
     ...
     | '<!---=====================================================--->'/
     | '<hr>'/
     | '<font color="red">MeteoMonitor</font> v2.7 (2001-oct-17) --'/
     | 'Copyright&#169 (1997) '/
...
C----------------------------------------------------------------------     
 ...
 8801 write(6,*) 'ERROR: meteo.config not found'
      write(6,*) '       this file contains the default URLs etc...'
      goto 8888

 8802 write(6,*) 'ERROR reading meteo.config'
      write(6,*) '      not enough lines'
      goto 8888

 8803 write(6,*) 'ERROR reading dimm.dat'
      goto 8888

 8804 write(6,*) 'ERROR reading ut.last'
      goto 8888
...
C     draw all the labels of a box
      ...
      implicit none
      real*4 x(10)              ! junk used for lines, labels, etc...
...
c      call sm_expand(1.01)

c      x(1) = 0. ! midnight line
c      x(2) = 0.
c      y(1) = y_min
c      y(2) = y_max
c      call sm_ctype('green')
