...
*
*.AUTHOR O.R. Hainaut
*.VERSION 2014-03-31 oh fixed missing average, 
*                       removed useless boxes (extn, RMS)
*.HISTORY
*     2012-05-27 oh corrected pressure values with  
*                keisan.casio.com/has10/SpecExec.cgi
*     2010-01-10 ohainaut maintenance and cleanup v2.9
...
*+++++++++++++++.
      ...
      real*4 SeeDimm(1000), cloud(1000),acloud(1000)
      ...
      integer LUUT, LUMsg, LUlos                ! logical units 
      ...
      integer iarrowtmp, iarrowhum, iarrowdpt ! arrows
      integer iarrowphg, iarrowwnd, iarrowwdir, iarrowcloud
      integer whicharrow        ! function to chose arrow
      integer flagWntt, flagW3p6, flagHum

      character ServUtHmChar*4
      character arrow(5)*8      ! hex unicode numbers of up/down arrows
      real*4 limWntt(2), limW3p6(2) ! wind limits
      real*4 UTlimWntt, UTlimW3p6 ! last time of limit
      ...
      real*4 cloud_min, cloud_max, cloud1, cloud2, cloud3 ! cloud limits
...
C     REAL*8!!!
      ...
      character wincol*32,tmpcol*32, phgcol*32, humcol*32,cloudcol*32
...
c----------------------------------------------------------------------
     ...
     $     '/lasilla/dimm/meteo.html'
      ...
      wnd1 = 12.                ! wind limits: safety
      wnd2 = 20.
      limWntt(1) = 12.          ! specific limits for main telesc.
      limWntt(2) = 15.
      limW3p6(1) = 14.
      limW3p6(2) = 20.
      ...
      phg1 = 768.               ! pressure limits
      phg2 = 765.               ! (values from Manfred Mornhingweg)
      phg_min = 762.5              ! 2012May: adjusted using
      phg_max = 773.               !  http://keisan.casio.com/has10/SpecExec.cgi
      ...
      obslong = -70.73d0        ! observatory longitude  (<0 West) (for Sun)
      obslat = -29.375d0        ! (3.6m at zenith)
...
c      screen_dev(1) = 'x11 -bg black -fg white -geometry 1200x740'
      ...
      expand_def = 1.0  ! 0.8          ! normal size for fonts and symbols
...
C no user serviceable parts beyond here
      ...
      arrow(1) = "&#x219F;" !  UPWARDS TWO HEADED ARROW
      arrow(2) = "&#x2197;" !  NORTH EAST ARROW 
      arrow(3) = "&#x2192;" !  RIGHTWARDS ARROW
      arrow(4) = "&#x2198;" !  SOUTH EAST ARROW
      arrow(5) = "&#x21A1;" !  DOWNWARDS TWO HEADED ARROW 
...
c
     ...
     $        tmp_max = dpt(i)
         if (phg_min.gt.phg(i).and.phg(i).gt.740.) phg_min = phg(i)     
         if (phg_max.lt.phg(i).and.phg(i).lt.800.)
     $        phg_max = phg(i)+0.5
     ...
     $        see_min = SeeDimm(i)
         ...
         if (cloud_min.gt.cloud(i).and.cloud(i).gt.-200)
     |        cloud_min = cloud(i)
         if (cloud_max.le.cloud(i)) cloud_max = cloud(i)
...
c*            xabs(i) = xabs(i)/airm(i)+0.1 ! airmass correction for the
c*                                ! absorption.  
c*            xabs_rms(i) = xabs_rms(i)/airm(i) ! these are from Marc
c*                                ! M Sarazin's IDL routine - He must know what
c*                                ! he is doing!   
c*            if (abs_min.gt.xabs(i)) abs_min = xabs(i)

C-2014-03: we don't have Abs nor Abs_RMS anymore....: set them to -9999
            xabs(i) = -9999.    ! no seeing monitor
            xabs_rms(i) = -9999.
...
C
C--   Compute the average Temp, Hum, etc...
C     This used to be provided by the weather station. 
C     Not anymore, so we compute it from the data file.


      do j = 1, npt-12
         atmp(j) = 0.           ! init for the average
         ahum(j) = 0.
         adpt(j) = 0.
         aphg(j) = 0.
         awnd(j) = 0.
         ...
         do i = 0, 11           ! average of the last 12 data points, ~15min
            atmp(j) = atmp(j) + tmp(i+j)
            ahum(j) = ahum(j) + hum(i+j)
            adpt(j) = adpt(j) + dpt(i+j)
            aphg(j) = aphg(j) + phg(i+j)
            awnd(j) = awnd(j) + wnd(i+j)
            awdir(j) = awdir(j) + wdir(i+j)
            acloud(j) = acloud(j) + cloud(i+j)
         enddo
         ...
         awnd(j)  = awnd(j) /12.
         awdir(j) = awdir(j)/12.        
         acloud(j) = acloud(j)/12.
      enddo   ! end average loop


C     - get recent trends for arrows
      work = (tmp(1)+20.)/(atmp(1)+20.)
      iarrowtmp = whicharrow(work)
      work = hum(1)/ahum(1)
      iarrowhum = whicharrow(work)
      work = (dpt(1)+20.)/(adpt(1)+20.)
      iarrowdpt = whicharrow(work)
      work = phg(1)/aphg(1)
      iarrowphg = whicharrow(work)
      work = wnd(1)/awnd(1)
      iarrowwnd = whicharrow(work)
      work = cloud(1)/acloud(1)
      iarrowcloud = whicharrow(work)


C
C--  Check warning msg
C
      do i = npt, 1, -1 ! start from the past
         if ((hour(1)-hour(i)).lt.0.5) then ! we consider only the last 1/2h

C     -NTT WIND
            if (awnd(i) .gt. limWntt(2)) then
               flagWntt = 2
               UTlimWntt = hour(i)
            else if (awnd(i) .gt. limWntt(1)) then
               if (flagWntt .le. 1) then
                  flagWntt = 1
                  UTlimWntt = hour(i)
               endif
            endif
C     -3P6 WIND
            if (awnd(i) .gt. limW3p6(2)) then
               flagW3p6 = 2
               UTlimW3p6 = hour(i)
            else if (awnd(i) .gt. limW3p6(1)) then
               if (flagW3p6 .le. 1) then
                  flagW3p6 = 1
                  UTlimW3p6 = hour(i)
               endif
            endif
...
C--   pressure
      ...
      y(1) = 770.               ! Average pressure at La Silla
      y(2) = 770.
...
C--   seeing
      if (debug) write(6,*)"DBG: start seeing"
      ...
      call sm_ptype(dot,npt)   ! dimm points
      call sm_expand(2.)
      call sm_points(utdimm,seeDimm,npt)


     
      call sm_ctype('green')   ! big green dimm point
      dot(1) = 83.7
      call sm_ptype(dot,npt) 
      call sm_expand(2.)
      call sm_points(utdimm,seeDimm,1)
      call sm_ctype(default_col(idev))
      call sm_expand(1.00)

      if (SeeDimm(1).le.0.0001) then
         call sm_ctype('yellow')
         call sm_expand(expand_small)    
         call sm_relocate(hour_now,0.1)
         call sm_putlabel(7,'\t Currently not available ')
         call sm_ctype(default_col(idev))
         call sm_expand(expand_def)    
      endif

      
      label20 = 'Seeing (")'    ! title and all normal labels
      call labeline(iut,hour_now,hour_left,y_min,y_max,label20)
      call sm_expand(expand_def)    



      call sm_ctype(default_col(idev)) ! reset 
      call sm_expand(expand_def)    



C*C--   Transparency
C*      if (debug) write(6,*)"DBG: start transp"
C*      call oli_window(iwin(1),iwin(2),3,3) 
C*
C*      iut = -1
C*
C*      y_min = abs_min
...
C*      x(2) = hour_now
C*
C*      call sm_limits(hour_left,hour_now,y_min,y_max)
C*      call sm_ticksize(1.,3.,0.,0.)
C*      call sm_box(0,1,0,1)
C*
C*      if (y_max.gt.abs1) then
C*         y(1) = abs1            ! abs warning line
C*         y(2) = abs1
C*
C*         call sm_ctype(linecol) 
C*         call sm_conn(x,y,2)
C*         call sm_relocate(x(1),y(1)+(y_max-y_min)*label_off)
C*         call sm_expand(expand_small)
C*         call sm_putlabel(9,' \t Clouds?')
C*         call sm_ltype(0)      ! solid
C*      endif
C*
C*
C*      if (y_max.gt.abs2) then
C*         x(1) = hour_left       ! abs warning line
C*         x(2) = hour_now
C*         y(1) = abs2
C*         y(2) = abs2
C*
...
C*
C*      call sm_ctype(default_col(idev)) 
C*      call sm_ptype(dot,npt)   ! plot the curve
C*      call sm_expand(2.)
C*      call sm_points(hour,xabs,npt)
C*
C*
C*      if (xabs(1) .lt. abs1 ) then ! chose the color of the big dot
C*         call sm_ctype('green')
C*         abscol = green
C*      else if (xabs(1).lt. abs2)  then
C*         call sm_ctype('yellow')
C*         abscol = yellow
C*      else
C*         call sm_ctype('red')
C*         abscol = red
C*      endif
C*      call sm_points(hour(1),xabs(1),1)
C*      call sm_ctype(default_col(idev))
C*      call sm_expand(1.00)
C*
C*
C*      label20 = 'Extinction (mg/airm)'
C*      call labeline(iut,hour_now,hour_left,y_min,y_max,label20)
C*      call sm_expand(expand_def)    
C*
C*      if (xabs(1).le.0.) then
C*         call sm_ctype('yellow')
C*         call sm_relocate(hour_now,0.03)
C*         call sm_expand(expand_small)    
C*         call sm_putlabel(7,'\t Currently not available ')
C*         call sm_ctype(default_col(idev))
C*         call sm_expand(expand_def)    
C*      endif
C*
C*C--   Transparency RMS
C*      if (debug) write(6,*)"DBG:   start rms"
C*      
C*      iut = 0
C*
C*      call oli_window(iwin(1),iwin(2),3,2) 
C*
C*      y_min = abs_rms_min
C*      y_max = abs_rms_max
C*
...
C*      call sm_box(0,1,0,1)
C*
C*      if (y_max.gt.abs_rms1) then
C*         y(1) = abs_rms1            !  warning line
C*         y(2) = abs_rms1
C*         call sm_ctype(linecol) 
C*         call sm_conn(x,y,2)
C*         call sm_relocate(x(1),y(1)+(y_max-y_min)*label_off)
...
C*      endif
C*
C*      if (y_max.gt.abs_rms2) then
C*         x(1) = hour_left       ! abs warning line
C*         x(2) = hour_now
C*         y(1) = abs_rms2
C*         y(2) = abs_rms2
C*
C*c         call sm_ltype(1)      ! dots
C*         call sm_ctype(linecol) 
C*         call sm_conn(x,y,2)
C*         call sm_relocate(x(1),y(1)+(y_max-y_min)*label_off)
C*         call sm_expand(expand_small)
...
C*      call sm_expand(2.)
C*      call sm_ctype(default_col(idev))
C*      call sm_points(xabs_ut,xabs_rms,nabs)
C*
C*      if (xabs_rms(nabs) .lt. abs_rms1 ) then ! chose the color of the big dot
C*         call sm_ctype('green')
C*         abs_rmscol = green
C*      else if (xabs_rms(nabs).lt. abs_rms2)  then
C*         call sm_ctype('yellow')
C*         abs_rmscol = yellow
C*      else
C*         call sm_ctype('red')
C*         abs_rmscol = red
C*      endif
C*      call sm_points(xabs_ut(nabs),xabs_rms(nabs),1)
C*
C*      call sm_ctype(default_col(idev))
C*      call sm_expand(1.00)
C*
C*      label20 = 'Extinction RMS'
C*      call labeline(iut,hour_now,hour_left,y_min,y_max,label20)
C*      call sm_expand(expand_def)    
C*
...
C      if (xabs(1).le.0..or.
C     |    xabs_rms(1).le.0.) then
C         call sm_ctype('yellow')
C         call sm_relocate(hour_now,0.003)
C         call sm_expand(expand_small)    
C         call sm_putlabel(7,'\t Currently not available ')
C         call sm_ctype(default_col(idev))
C         call sm_expand(expand_def)    
...
CC       if (flagWntt .gt. 0) then
CC          call oli_window(3,6,3,3) ! bottom part 
CC          call sm_limits(hour_left,hour_now,y_min,y_max)
CC          call sm_ticksize(0.,1.,0.,1.)
CC          if (flagWntt .eq. 2) then
CC             call sm_ctype(red)
CC          call sm_box(0,0,0,0)
CC       endif
CC 
...
C--   Clouds
      ...
      if (debug) write(6,*)"DBG: start Clouds Pyrgeometer"
      call sm_expand(expand_def)    
      call oli_window(iwin(1),iwin(2),3,2) 

      iut = -1  ! UT on top of box
      ...
      call sm_relocate(x(1),y(1)-(y_max-y_min)*label_off)
      call sm_expand(expand_small)
      call sm_putlabel(3,' \t Clear')

      x(1) = hour_left
      y(1) = cloud2 ! some clouds
      y(2) = cloud2
      call sm_ctype(linecol)
      call sm_conn(x,y,2)
      call sm_relocate(x(1),y(1)+(y_max-y_min)*label_off)
      ...
      call sm_relocate(x(1),y(1)+(y_max-y_min)*label_off)
      call sm_expand(expand_small)
      call sm_putlabel(9,' \t Thick clouds')
      
      call sm_ctype(default_col(idev))
      call sm_ptype(dot,npt)
      call sm_expand(2.)
      call sm_points(hour,cloud,npt)

      if (cloud(1) .gt. y_ok) then
         call sm_ctype('green')
         cloudcol = green
      else if (cloud(1) .gt. y_danger)  then
         call sm_ctype('yellow')
         cloudcol = yellow
      else
         call sm_ctype('red')
...
!-- bottom labels 
      ...
      if (idev .eq. 2) then
         ...
         call sm_expand(expand_small)
         call sm_putlabel
     |        (1,'\t Meteomonitor 2.9; comments: ohainaut@eso.org')
...
*     *Sidereal time
     ...
     >     + 0.093104d0*dTU*dTU - 6.2d-6*dTu*dTu*dTu
      ...
      dST = dST0ut + (hh(1) + mm(1)/60.d0 )/.9972696d0
...
c-- print the header      
 ...
 5012 format(
     ...
     | '      vlink="ddddff"> '/
     | ' '/
     | '<a name="tab"><p><h1>La Silla - MeteoMonitor</h1>'/
     | '<!---=====================================================--->'/
     | '<!---====   table     ====================================--->'/
     | '<!---=====================================================--->'/
 ...
 5013 format(
     ...
     | '      vlink="ddddff"> '/
     | ' '/
     | '<p><h1>La Silla - MeteoMonitor</h1>'/
     | '<!---=====================================================--->'/
     | '<!---====   table     ====================================--->'/
     | '<!---=====================================================--->'/
     | '<p>'/
     | '<table border=1>               <!---Small Inner table ------->'/
     | '<!--row 1 = header ------------------------------------------>'/
     | ' <tr><th></th>'/                     ! Name    1
     | '     <th>Current</th>'/              ! Current 2
     | '     <th> </th>'/                     ! color ball b
...
c--   fill the table
      write(LUHtmLoc,5030)
     $     hum(1),arrow(iarrowhum),humcol,ahum(1)
      write(LULightHtmLoc,5030)
     $     hum(1),arrow(iarrowhum),humcol,ahum(1)
      write(LUZommHtmLoc,5030)
     $     hum(1),arrow(iarrowhum),humcol,ahum(1)
 5030 format(
     | '<!--row 2 = Humidity   ------------------------------------->'/
     ...
     |     )
      ...
      write(LUHtmLoc,5020)
     $     tmp(1),arrow(iarrowtmp),tmpcol,atmp(1)
      write(LULightHtmLoc,5020)
     $     tmp(1),arrow(iarrowtmp),tmpcol,atmp(1)
      write(LUZommHtmLoc,5020)
     $     tmp(1),arrow(iarrowtmp),tmpcol,atmp(1)
 5020 format(
     | '<!--row 3 = Temperature ------------------------------------->'/
     ...
     | )
      ...
      write(LUHtmLoc,5040)
     $     dpt(1),arrow(iarrowdpt),tmpcol,adpt(1)
      write(LULightHtmLoc,5040)
     $     dpt(1),arrow(iarrowdpt),tmpcol,adpt(1)
      write(LUZommHtmLoc,5040)
     $     dpt(1),arrow(iarrowdpt),tmpcol,adpt(1)
 5040 format(
     | '<!--row 4 = Dew Point   ------------------------------------->'/
     ...
     |)
      ...
      write(LUHtmLoc,5050)
     $     wnd(1),arrow(iarrowwnd),wincol,awnd(1)
      write(LULightHtmLoc,5050)
     $     wnd(1),arrow(iarrowwnd),wincol,awnd(1)
      write(LUZommHtmLoc,5050)
     $     wnd(1),arrow(iarrowwnd),wincol,awnd(1)
 ...
 5050 format(
     ...
     |)
      ...
      write(LUHtmLoc,5060)
     $     phg(1),arrow(iarrowphg),phgcol,aphg(1)
      write(LULightHtmLoc,5060)
     $     phg(1),arrow(iarrowphg),phgcol,aphg(1)
      write(LUZommHtmLoc,5060)
     $     phg(1),arrow(iarrowphg),phgcol,aphg(1)
 ...
 5060 format(
     ...
     |)
      ...
      write(LUHtmLoc,5061)
     $     cloud(1),arrow(iarrowcloud),cloudcol,acloud(1)
      write(LULightHtmLoc,5061) 
     $     cloud(1),arrow(iarrowcloud),cloudcol,acloud(1)
      write(LUZommHtmLoc,5061)  
     $     cloud(1),arrow(iarrowcloud),cloudcol,acloud(1)


 5061 format(
     | '<!--row 6 = Pyrgeometer    ------------------------------->'/
     | ' <tr><td align=right>Clouds:</td>'/  ! 1
     | '     <td align=center><font color="ffffff">&#032;',f6.1,/
     | ' ',a8,' </font></td>'/          ! 2
     |     '     <td><img src="',a32,'" width=20 height=20></td>'/ ! b
     | '     <td align=center>',f6.1,'</td>'/  ! 3
 ...
 5062 format(
     ...
     |)
      ...
      write(LUHtmLoc,5070)      SeeDimm(1)
      write(LULightHtmLoc,5070) SeeDimm(1)
      write(LUZommHtmLoc,5070)  SeeDimm(1)
 5070 format(
     | '<!--row 8 = Seeing       ------------------------------------>'/
     | ' <tr><td align=right>DIMM Seeing:</td>'/
     | '     <td align=center>',f4.2,'</td>'/   
     | '     <td> </td>'/
     | '     <td><td >arcsec</td>'/
     | ' </tr>  '/
     |)
...
C     Check UT consistency and print message
      ...
      if (servUT.ge.0.) then ! valid time from server
         UTdiff = abs(servUT - hour(1))
         if (UtDiff .gt. maxUtDiff)then
            write(LUHtmLoc,3102)      servUtH,servUtM
            write(LULightHtmLoc,3102) servUtH,servUtM
            write(LUZommHtmLoc,3102)  servUtH,servUtM
         endif
      endif
 3102 format(
     | '<!--row 15 = UT Warning  ----------------------------------->'/
...
c     | '<hr>'/
     ...
     | '<a href="/lasilla/dimm/lasc">'//
     | 'La Silla all-sky camera</a> |'/
     | '<a href="http://allsky-dk154.asu.cas.cz/">'/
     | 'Danish Telescope all-sky camera</a> |'/
     | '<a href="/lasilla/sciops/',
     | '2p2/EnvMon/2p2/">',
     | 'ESO/MPI 2.2m Tel. Environment Monitor</a>|',
     | '<a href="http://www.eso.org/~ohainaut/meteo/meteomonitor.html"'/
     $ '>Garching MeteoMonitor</a>',
     | '] '/
     | '<!---=====================================================--->'/
     | '<!---====   Copyright ====================================--->'/
     | '<!---=====================================================--->'/
     | '<hr>'/
     | '<font color="red">MeteoMonitor</font> v2.10 (2014-Mar-31) --'/
     | 'Copyright&#169 (1997) '/
     | '<a href="http://www.eso.org">'/
     | 'European Southern Observatory</a>/'/
     | '<a href="http://www.sc.eso.org/~ohainaut">'/
     | 'O.Hainaut</a>.<br>'/
     |)
...
C     --- label the X axis with hours in [0,23]
 ...
 2999       format(i2.2)
            ...
            if (mod(i,3).eq.0) then
               ...
               call sm_putlabel(5,line(1:3))
            endif
         enddo
         
         call sm_relocate(hour_now,ylabel1)
         call sm_putlabel(3,' \t UT')
...
C==============================================================================
      function whicharrow(work)
C     decide which up/down arrow should be used
C------------------------------------------------------------------------------
      integer whicharrow
      real*4 work

      if (work .gt. 1.05) then
         whicharrow = 1
      else if (work .gt. 1.02) then
         whicharrow = 2
      else if (work .ge. 0.98) then
         whicharrow = 3
      else if (work .ge. 0.95) then
         whicharrow = 4
      else
         whicharrow = 4
      endif
